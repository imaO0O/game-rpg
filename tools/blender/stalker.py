...
def build_head():
    ...
    # Уши: выше, чем шире, и развёрнуты по касательной к черепу.
    # Приклеенные плашмя читались лопастями, торчащими вбок.
    for side in (-1, 1):
        parts.append(sphere("ear", 0.03, (side * 0.094, -0.04, HEAD_Z - 0.008),
                            (0.22, 0.62, 1.0),
                            (0, 0, side * math.radians(18))))
...
def build_jacket():
    ...
    joints = [
        ...
        ((-0.19, -0.04, 1.45), 0.078, 0.074),
        ((-0.235, 0.0, 1.18), 0.07, 0.068),
        ((-0.25, 0.02, 0.98), 0.055, 0.055),

        ((0.19, -0.04, 1.46), 0.078, 0.074),
    ...
    # Воротник-стойка.
    parts.append(tube("collar", 0.082, 0.16, (0.0, -0.052, 1.63), verts=20))
...
def build_legs_gear():
    ...
    for side in (-1, 1):
        ...
        parts.append(box("boot_vamp", (0.096, 0.15, 0.08),
                         (x * 1.15, -0.05, 0.06), bevel=0.014))
        # Носок отдельной коробкой с почти вертикальной передней гранью.
        parts.append(box("boot_toe", (0.094, 0.075, 0.055),
                         (x * 1.15, -0.115, 0.045), bevel=0.012))
        parts.append(box("boot_sole", (0.096, 0.205, 0.024),
                         (x * 1.15, -0.05, 0.014), bevel=0.006))
