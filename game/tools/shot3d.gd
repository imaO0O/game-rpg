...
const VIEWS := [
	...
	# Ближе к шкафу и лампе: с прежней точки между объективом и светом
	# было пять метров тумана, и двойное затухание съедало кадр.
	# Кровать и шкаф в правой трети кадра, а не за спиной: с прежней
	# точки в кадр попадала одна полка и много пустоты.
	{"name": "f_far_room", "pos": Vector3(4.7, 1.05, -11.0), "yaw": -1.1, "pitch": -0.08},
	...
	# Довёрнут вправо: доска должна открываться центральным узлом,
	# а силуэт занимать левую треть кадра.
	{"name": "j_finale", "pos": Vector3(-4.35, 1.35, -15.2), "yaw": -0.28, "pitch": -0.05},
]
