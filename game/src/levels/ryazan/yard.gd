...
## Планировка двора без наполнения.
##
## Вынесена отдельно не ради красоты: ночная Рязань (DESIGN.md §9) берёт
## её целиком. Тот же двор, те же скамейки, всё не так — приём работает
## только если геометрия буквально та же, а не похожая.
func layout() -> void:
	bounds(0, 0, 56, 22)
