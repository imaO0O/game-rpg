...
func _build_zade_room() -> void:
	...
	# Смещён с оси доски: стоя по центру, он закрывал собой самый
	...
	# Слабее и с более крутым спадом, чем кажется нужным: при 0.9 свет
	# добивал до доски и выбеливал записки — то есть съедал именно то,
	# ради чего сцена и поставлена.
	_point_light(Vector3(-4.2, 1.75, -17.5), Color(1.0, 0.82, 0.6), 0.55, 2.4, 3.2)
